...
public class UpdaterAPI {
   ...
   */
  public UpdaterAPI(JavaPlugin plugin, String pluginurl) {
    try {
      url = new URL(pluginurl);
    } catch (MalformedURLException e) {
      canceled = true;
      plugin.getLogger()
          .log(Level.WARNING, "Error: Bad URL while checking {0} !", plugin.getName());
    }
    this.plugin = plugin;
  ...
  public boolean needsUpdate() {
    ...
    try {
      ...
      if (newVersionAvailiable(plugin.getDescription().getVersion(),
          version.replaceAll("[a-zA-z ]", ""))) {
        if (out) {
          plugin.getLogger()
              .log(Level.INFO, " New Version found: {0}", version.replaceAll("[a-zA-z ]", ""));
      ...
      plugin.getLogger()
          .log(Level.WARNING, "Error in checking update for ''{0}''!", plugin.getName());
  ...
  public void update() {
    try {
      ...
      try {
        if (out) {
          plugin.getLogger().log(Level.INFO, "Trying to download from: {0}{1}",
              new Object[]{getFolder(pluginurl), downloadURL});
        }
      ...
      if (out) {
        ...
        plugin.getLogger()
            .log(Level.INFO, "To install the new features you have to restart your server!");
      }
    } catch (IOException e) {
  ...
  /**
   * Executes the Update and trys to install it But uses an external link for downloading the File
   */
